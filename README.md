...
> 💡 **特别推荐**：虽然 Panel Composer 不限定使用场景，但它特别适合**科研工作者在论文绘图时使用**——把散落的子图（panel）快速组合成 Figure（如 Figure 1A-F），省去在 Illustrator 里逐个对齐的繁琐操作。

### 🎯 科研绘图利器

虽然 Panel Composer 不限定具体使用场景，但它**特别适合科研工作者在论文绘图时使用**：

- **批量处理**：一句话组合 Figure 1~10，不用逐个手动操作
- **PDF 矢量输出**：投稿级别质量，放大不失真
- **等比缩放保护**：panel 内容不会被拉伸变形，符合期刊要求
- **标签系统**：自动生成 A/B/C 或 1/2/3 标签
- **快速迭代**：AI 自动排 → 对话微调 → 画布精调，三步到位
- **专业软件衔接**：导出 PDF 后可在 Affinity/Illustrator 中继续精修
...
如果你的 panel 文件名是 `result1.png`、`heatmap.pdf` 这种没有 A/B/C/D 的命名，也完全没关系。你不用自己打开图片、逐个手动加编号——直接让 AI 根据图片内容判断叙事顺序并自动重命名：

> "根据这几张图的内容，按从方法到结果的叙事逻辑排个顺序，自动给它们编上 A、B、C、D"

AI 会读懂每张图在讲什么，决定哪个该放前面、哪个放后面，并自动打上标签。你全程只需要说一句话，编号和顺序都由 AI 搞定。
...
默认所有 PDF panel **保持矢量**（可无损放大，适合在 Affinity/Illustrator 中继续编辑）。

如果某个子图元素特别多（比如热图、散点图），可以单独转成位图：

> "把热图转成位图"

> "C 子图元素太多，转位图"

> "第 1 个和第 3 个 panel 转位图"
...
| `rasterize` | bool/list | `False` | `False`=全矢量；`True`=全位图(600DPI)；`[0,2]`=指定索引转位图 |
